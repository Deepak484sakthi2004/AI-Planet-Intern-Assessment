...
def hallucination_grader(documents,generation,docs):
        prompt_template =  f"""
                        ...
                        <eot_id><start_header_id>assistant<end_head_id>
                    """
        prompt = PromptTemplate(template=prompt_template,  input_variables = ["generation","documents"])
        model = ChatGoogleGenerativeAI(model="gemini-1.5-pro", temperature=0.3, google_api_key=gemini_api)
        hallucination_grader_chain = load_qa_chain(model, chain_type="stuff", prompt=prompt, document_variable_name="context")
